    ...
    def POST(self):
        try:
            ...
            if msg.type in ["text", "voice", "image"]:
                ...
                if task_running:
                    if request_cnt == 1:
                        # 第一次请求且任务仍在处理中：立即返回“正在检索中”的提示
                        # 后台线程会继续异步调用外部 RAG / 大模型，用户稍后再发任意消息即可拿到结果
                        reply_text = "【已开始调用外部知识库检索，处理可能需要几十秒。稍后回复任意文字以获取本次问题的完整答案】"
                        replyPost = create_reply(reply_text, msg)
                        return encrypt_func(replyPost.render())
                    elif request_cnt < 3:
                        # 第二、三次重试且仍未完成，直接返回 success，不重复提示
                        time.sleep(2)
                        return "success"
                    else:  # 理论上很少走到
                        reply_text = "【检索仍在进行中，请稍后回复任意文字以获取结果】"
                        replyPost = create_reply(reply_text, msg)
                        return encrypt_func(replyPost.render())
