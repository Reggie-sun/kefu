...
```json
{
  ...
  "open_ai_api_base": "https://your-azure-openai-endpoint/",
  "azure_deployment_id": "YOUR_DEPLOYMENT_NAME",
...
```json
{
  ...
  "open_ai_api_key": "YOUR_DASHSCOPE_COMPATIBLE_API_KEY"
}
```
- `bot_type`: OpenAI兼容方式
...
## 🚀 docker-compose 一键起（机器人/网关/向量库/缓存/看板）

```bash
cp .env.example .env
# 填写 OpenAI/其他 LLM key、channel 等配置
docker-compose up -d
```

包含的服务：
- bot：聊天机器人（channel 取决于 `.env` 中 `CHANNEL_TYPE`）
- gateway：Smart Gateway（RAG + 工具 + 日志）
- vector-db：pgvector（可换成本地/托管向量库）
- redis：可选会话存储
- dashboard：FastAPI + 原生前端看板（端口 8501）

可调参数（`.env`）：
- `GATEWAY_LOG_DB`：默认 `sqlite:///data/gateway_logs.sqlite3`；生产可改 Postgres。
- `SMART_ROUTING_MODE`：`rule_based`/`react`
- `SMART_RERANK`：`true`/`false`
- `VECTOR_DB_*`：pgvector 凭据
